    ...
    try {
      ...
      if (selectedFile) {
        try {
          ...
          if (!imgbbKey) {
            throw new Error("Chave da API do ImgBB não configurada em Variables de Ambiente.");
          }
          ...
          if (data.success) {
            ...
            throw new Error(data.error?.message === 'Invalid API v1 key.' 
              ? 'Chave da API ImgBB inválida. Acesse as Configurações (Settings) no AI Studio e adicione uma chave válida na variável VITE_IMGBB_API_KEY.' 
              : data.error?.message || "Falha ao processar o upload da imagem.");
          }
        } catch (uploadErr: any) {
